...
import { Cpu, ArrowRight, ArrowLeft, Cylinder } from 'lucide-react';
...
export default function Step3SetupSizing({ wizardData, updateWizardData, onNext, onPrev }) {
  // Safe extraction of values from wizardData with fallbacks
    ...
    <div className="space-y-6 animate-fade-in text-left">
      ...
      <div className="glass-card rounded-2xl p-5 border border-slate-800 flex items-center justify-between">
        ...
        <div className="px-3.5 py-1.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-xs font-mono font-bold text-emerald-400">
          Target Storage: {(hydro.tankCapacity || 0).toLocaleString()} L
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
                ...
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between py-1.5 border-b border-slate-800">
                    ...
                    <span className="font-mono font-bold text-sky-300">
                      {(hydro.tankCapacity || 0).toLocaleString()} Liters
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between py-1.5 border-b border-slate-800">
                    ...
                    <span className="font-mono font-bold text-slate-200">{hydro.downpipeDiameter || '110 mm'}</span>
                  </div>

                  <div className="flex justify-between py-1.5 border-b border-slate-800">
                    <span className="text-slate-400">Estimated Gutters:</span>
                    <span className="font-mono font-bold text-slate-200">{hydro.gutterLength || 0} meters</span>
                  </div>

                  <div className="flex justify-between py-1.5">
                    <span className="text-slate-400">Recharge Pit Size:</span>
                    <span className="font-mono font-bold text-amber-300">{hydro.pitDimensions || 'N/A'}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="glass-card p-4 rounded-xl border border-slate-800">
              ...
              <div className="text-sm font-extrabold text-white">{hydro.downpipeDiameter || '110 mm'}</div>
            ...
            <div className="glass-card p-4 rounded-xl border border-slate-800">
              ...
              <div className="text-sm font-extrabold text-white">{hydro.pitVolumeM3 || 0} m³ Recharge</div>
              <p className="text-[10.5px] text-slate-400 mt-1">Pumps clean excess runoff directly to groundwater table.</p>
            </div>
          </div>
        </div>

        {/* Right Col: Itemized Costing Table */}
          ...
          <div>
            ...
            <div className="space-y-3 mb-5">
              <div className="flex justify-between items-center text-xs py-1.5 border-b border-slate-800">
                <span className="text-slate-300">Polyethylene Tank ({hydro.tankCapacity || 0}L)</span>
                <span className="font-mono font-bold text-white">₹ {(hydro.pricing?.tankCost || 0).toLocaleString()}</span>
              </div>

              <div className="flex justify-between items-center text-xs py-1.5 border-b border-slate-800">
                <span className="text-slate-300">Multi-Stage Charcoal Filter</span>
                <span className="font-mono font-bold text-white">₹ {(hydro.pricing?.filterCost || 0).toLocaleString()}</span>
              </div>

              <div className="flex justify-between items-center text-xs py-1.5 border-b border-slate-800">
                <span className="text-slate-300">PVC Pipes & Gutters ({hydro.gutterLength || 0}m)</span>
                <span className="font-mono font-bold text-white">₹ {(hydro.pricing?.pipingCost || 0).toLocaleString()}</span>
              </div>

              <div className="flex justify-between items-center text-xs py-1.5 border-b border-slate-800">
                <span className="text-slate-300">Pit Excavation & Gravel Liner</span>
                <span className="font-mono font-bold text-white">₹ {(hydro.pricing?.excavationCost || 0).toLocaleString()}</span>
              </div>

              <div className="flex justify-between items-center text-xs py-1.5 border-b border-slate-800">
                <span className="text-slate-300">Labor & Plumbing Installation</span>
                <span className="font-mono font-bold text-white">₹ {(hydro.pricing?.laborCost || 0).toLocaleString()}</span>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 text-center">
              ...
              <div className="text-3xl font-extrabold text-sky-400 font-mono tracking-tight mt-1">
                ₹ {(hydro.pricing?.totalCost || 0).toLocaleString()}
              </div>
