/**
 * SAVJAL Hydrological Calculation Engine (Server-Side)
 * ────────────────────────────────────────────────────
 * Server-side mirror of the frontend calculation engine.
 * Independently verifies all hydrological computations
 * before embedding them into the Municipal PDF report.
 *
 * Standards: CGWB (Central Ground Water Board) India
 * Baseline: 135 LPCD (Liters Per Capita per Day)
 * Conversion: 1 mm rain on 1 m² = 1 Liter
 */

function calculateHydrology(roofArea, rainfall, runoffCoeff = 0.85, householdMembers = 4, soilType = 'Loamy') {
  ...
  // ─── Household Demand (135 L/person/day standard) ────────
  ...
  // ─── Storage Tank Sizing (18% of annual, quantized 500L) ─
  const tankCapacity = Math.round(annualHarvest * 0.18 / 500) * 500 || 3000;

  // ─── Downpipe Sizing (area-based rules) ──────────────────
  ...
  if (soilType === 'Clayey') {
    safetyAlert = {
      ...
      message: 'Clay soil has poor percolation (< 10 mm/hr). Above-Ground Polyethylene Storage Tanks are recommended over Soil Recharge Pits to prevent surface waterlogging.'
    };
  ...
  // ─── 12-Month Water Balance (Indian Monsoon) ────────────
  ...
  const aiRecommendations = [
    ...
    `Roof surface (${area} m²) generates enough water to fulfill ${selfSufficiencyPct}% of your family's annual non-potable needs.`
  ];
...
module.exports = { calculateHydrology };
